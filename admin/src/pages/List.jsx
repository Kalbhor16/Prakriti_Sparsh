...
function List() {
  ...
  return (
    <div className='w-[100vw] min-h-[100vh] bg-gradient-to-r from-[#FF57B9] via-[#FF8FD2] to-[#FFC9E8] text-[white]'>
      <Nav/>
              ...
              list.map((item, index)=>(
                <div className='w-[90%] md:h-[120px] h-[90px] bg-slate-600 rounded-xl flex items-center justify-start gap-[5px] md:gap-[30px] p-[10px] md:px-[30px]' key={index}>
                  ...
                  <div className='w-[90%] h-[80%] flex flex-col items-start justify-center gap-[2px]'>
                    <div className='w-[100%] md:text-[20px] text-[15px] text-[#bef0f3]'>{item.name || 'Unnamed product'}</div>
                    <div className='md:text-[20px] text-[15px] text-[#bef0f3]'>{item.category || 'No category'}</div>
                    <div className='md:text-[20px] text-[15px] text-[#bef0f3]'>₹ {item.price ?? 'N/A'}</div>
                  </div>
